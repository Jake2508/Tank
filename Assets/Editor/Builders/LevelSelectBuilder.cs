...
public static class LevelSelectBuilder
{
    ...
    /// <summary>
    /// Top padding is well over the bottom on purpose.
    ///
    /// Both plate labels align to their baseline, which is what lands the tier word on
    /// the level name's baseline instead of floating it in the middle of the taller
    /// text. TMP puts that baseline at the vertical centre of the layout rect - and
    /// since both strings are all-caps with no descenders, every glyph then sits above
    /// the centre line and the text rides high in the slab. Pushing the rect down moves
    /// the baseline down with it, so the cap block ends up optically centred while the
    /// two labels stay locked to each other.
    /// </summary>
    const float PlatePadL = 15.5f, PlatePadR = 15.5f, PlatePadT = 15f, PlatePadB = 1f;
