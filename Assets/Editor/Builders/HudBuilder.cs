...
public static class HudBuilder
{
    ...
    static void BuildHullBar(RectTransform cluster, ref HullRefs refs)
    {
        ...
        // The trough doubles as a stencil for the fill.
        //
        // Body, trough and keyline are all parallelograms carrying the 7 degree lean,
        // but the fill is a tiled sprite in an axis-aligned rect - so at the two corners
        // where the shear runs against the rectangle (top-right and bottom-left) the
        // yellow overshot the slab and exposed the dark body and the cream keyline
        // behind it. Masking clips the fill to the slab's actual silhouette.
        //
        // The graphic has to live on the masking object itself, so the trough image
        // moves up onto Track and draws through the mask rather than under it.
        var troughImage = Paint(track, Hud + "hud_slab_fill.png", HudPalette.CreamTrough, Image.Type.Sliced);

        var mask = EnsureComponent<Mask>(track.gameObject);
        mask.showMaskGraphic = true;

        // A separate Trough child would now be drawn twice; retire it if an older build
        // of this scene left one behind.
        var legacyTrough = track.Find("Trough");
        if (legacyTrough != null) Object.DestroyImmediate(legacyTrough.gameObject);
    ...
    /// <summary>
    /// The HUD no longer owns the drift readout.
    ///
    /// This used to put a read-only DriftMeter on the canvas that derived drift from the
    /// player's slip angle and pushed it to HudController. DriftBoost on the tank now
    /// charges, spends and reports the meter, so leaving the old one here would give two
    /// components calling SetDrift every frame and whichever ran last would win - the
    /// pips would ignore the charge the boost actually holds.
    /// </summary>
    static void WireDriftSource(GameObject canvasGo)
    {
        // Matched by type name rather than by type, because the DriftMeter class itself
        // has been deleted. That keeps this guard working for anyone opening an older
        // scene without keeping a dead class alive purely so this line compiles.
        int removed = 0;
        foreach (var behaviour in canvasGo.GetComponentsInChildren<MonoBehaviour>(true))
        {
        ...
        if (removed > 0) Debug.Log("[HUD] Retired " + removed + " legacy DriftMeter(s).");
    }
