...
public static class TitleScreenCapture
{
    ...
    public static void Capture()
    {
        ...
        // TANKEO_CAPTURE_WEBGL=1 shows what the browser build looks like: Quit is a
        // no-op there, so it is greyed out. The gate itself is a compile-time one, so
        // this is the only way to see it without building for WebGL.
        var menu = Object.FindObjectOfType<MainMenuUI>();
        var quitSlab = FindSlab(canvasGo.transform, "QuitButton");
        bool posedWebGL = Env("TANKEO_CAPTURE_WEBGL", "0") == "1" && menu != null;
        if (posedWebGL) menu.EditorPoseNoQuit();
        ...
        finally
        {
            ...
            // Put the slab back, so a pose can never be saved into the scene.
            if (posedWebGL && quitSlab != null)
            {
                quitSlab.SetAvailable(true);
                if (quitSlab.Button != null) quitSlab.Button.interactable = true;
            }
    ...
    static MenuSlab FindSlab(Transform canvas, string slabName)
    {
        var rect = canvas.Find("MainMenuUI/" + slabName);
        return rect != null ? rect.GetComponent<MenuSlab>() : null;
    }
